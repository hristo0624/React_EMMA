...
class MatxVerticalNavExpansionPanel extends Component {
  ...
  componentDidMount() {
    ...
    this.listenRouteChange = this.props.history.listen((location, action) => {
        if (location.pathname != this.props.item.path && location.pathname.indexOf(this.props.item.path) > -1) {
            this.setState({ collapsed: false });
        }
    });
  }
