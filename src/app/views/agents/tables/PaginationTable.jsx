...
const Agents = (props) => {
  ...
  const [columns, setColumns] = useState([
    ...
    },
    {
      ...
      // validate: rowData => rowData.contact_email != '' && validateEmail(rowData.contact_email) ? '' : 'Invalid Email'
      validate: rowData => {
        if (!rowData.fuel_email || (rowData.fuel_email && rowData.fuel_email != '' && validateEmail(rowData.fuel_email))) {
          return ''
        } else {
          return 'Invalid Email'
        }
  ...
  const getTotalDocs = (search = '') => {
    if (search == '') {
      return new Promise((resolve, reject) => {
        Firestore.collection('organizations').where('organization_type', '==', '2').get().then(docs=> {
  ...
  const navigateDetailPage = (docId) => {
    props.history.push(`/agentDetail/${docId}`, {docId: docId});
  }
          ...
          onRowAdd: newData =>
            new Promise((resolve, reject) => {
              Firestore.collection('organizations').add({
                ...
                fuel_email: newData.fuel_email ? newData.fuel_email : '',
                organization_type: '2',
